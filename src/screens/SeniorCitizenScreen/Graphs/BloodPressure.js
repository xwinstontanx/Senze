...
export default class BloodPressure extends Component {
  ...
  componentDidMount() {
    ...
    dbRefUser
      ...
      .get()
      .then((queryData) => {
        queryData.forEach((bpData) => {
          if (parseInt(bpData.data().Systolic) > 0 && bpData.data().DeviceType === 3) {
            const HealthDataS = parseFloat(bpData.data().Systolic);
            const HealthDataD = parseFloat(bpData.data().Diastolic);
            const pulse = bpData.data().Pulse === undefined ? "--" : parseFloat(bpData.data().Pulse);

            let date = bpData.data().CreatedAt.toDate();
            let time =
            ...
            this.systolic.push({ Time: Time, HealthData: HealthDataS, Type: 'Systolic' });
            this.systolic.push({ Time: Time, HealthData: HealthDataD, Type: 'Diastolic' });
            this.systolic.push({ HealthData: pulse, Type: 'Pulse', Time: Time });
          }
        });
        this.setState({
          dataJson: this.systolic,
          schemaJson: Schema,
          isLoading: false,
        });
      });
