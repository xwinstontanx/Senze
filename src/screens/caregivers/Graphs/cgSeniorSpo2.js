...
export default class cgSeniorSpo2 extends Component {
  _isMounted = false;
  spo2 = [];
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      tableData: [],
      tableHead: [translate('RECORDED AT'), translate("SPO2") + ' (%)'],
      type: 'timeseries',
      width: '100%',
      height: '100%',
      dataFormat: 'json',
  ...
  componentDidMount() {
    ...
    dbRefUser
      ...
      .get()
      .then((queryData) => {
        queryData.forEach((spo2Data) => {
          if (parseInt(spo2Data.data().Spo2) > 0 && spo2Data.data().DeviceType === 1) {
            const HealthData = parseFloat(spo2Data.data().Spo2);

            let date = spo2Data.data().CreatedAt.toDate();
            let time =
            ...
            this.spo2.push({ HealthData: HealthData, Type: 'Spo2', Time: Time });
          }
        });
        this.setState({
          dataJson: this.spo2,
