...
export default class cgSeniorTemperature extends Component {
  _isMounted = false;
  temp = [];
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      tableData: [],
      tableHead: [translate('RECORDED AT'), translate("SPO2") + ' (\u00b0C)',],
      type: 'timeseries',
      width: '100%',
      height: '100%',
      dataFormat: 'json',
  ...
  componentDidMount() {
    ...
    dbRefUser
      ...
      .get()
      .then((queryData) => {
        queryData.forEach((tempData) => {
          if (parseFloat(tempData.data().Temperature) > 0.0 && tempData.data().DeviceType === 1) {
            const HealthData = parseFloat(tempData.data().Temperature);
            let date = tempData.data().CreatedAt.toDate();
            let time =
            ...
            this.temp.push({ HealthData: HealthData, Type: 'Temperature', Time: Time });
          }
        });
        this.setState({
          dataJson: this.temp,
