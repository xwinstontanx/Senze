...
export default class cgSeniorWeight extends Component {
  _isMounted = false;
  weight = [];
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      tableData: [],
      tableHead: [translate('RECORDED AT'), translate('WEIGHT (IN KG)')],
      type: 'timeseries',
      width: '100%',
      height: '100%',
      dataFormat: 'json',
  ...
  componentDidMount() {
    ...
    dbRefUser
      ...
      .get()
      .then((queryData) => {
        queryData.forEach((weightData) => {
          if (parseFloat(weightData.data().Weight) > 0.0 && weightData.data().DeviceType === 4) {
            const HealthData = parseFloat(weightData.data().Weight);
            let date = weightData.data().CreatedAt.toDate();
            let time =
            ...
            this.weight.push({ HealthData: HealthData, Type: 'Weight', Time: Time });
          }
        });
        this.setState({
          dataJson: this.weight,
          schemaJson: Schema,
          isLoading: false,
        });
      });
