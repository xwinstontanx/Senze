...
class cgAddMedication extends Component {
  _isMounted = false;
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      showReminder3: false,
      isThirdDatePickerVisible: false,
      setThirdDatePickerVisibility: false,
  ...
  componentDidMount() {
    ...
    time.setMinutes(0);
    this.setState({ Reminder1: time, Reminder2: time, Reminder3: time });
    ...
    if (this.props.navigation.state.params.id != null) {
      ...
      firestore().collection('Medication').doc(id).get().then((medication) => {
        ...
        if (secondReminder.length == 2) {
          ...
          this.setState({ Reminder2, showReminder2: true })
        }

        const thirdReminder = medication.data().Reminder3.split(':');

        if (thirdReminder.length == 2) {
          const Reminder3 = new Date();
          Reminder3.setHours(parseInt(thirdReminder[0]))
          Reminder3.setMinutes(parseInt(thirdReminder[1]))
          this.setState({ Reminder3, showReminder3: true })
        }
  ...
  handleConfirm = () => {
    ...
    const { MedicineName, Reminder1, Reminder2, Reminder3, Dosage, Unit, NumberOfTime, Meal, showReminder2, showReminder3 } = this.state;

    if (MedicineName.trim()) {
      const medication = {
        CreatedBy: this.state.senior.data.Uid,
        UpdatedAt: firestore.FieldValue.serverTimestamp(),
        MedicineName: MedicineName,
        Reminder1: this.convertDateObjectToTimeString(Reminder1),
        Reminder2: showReminder2 ? this.convertDateObjectToTimeString(Reminder2) : '',
        Reminder3: showReminder3 ? this.convertDateObjectToTimeString(Reminder3) : '',
              ...
              <View style={styles.form}>
                <View >
                  ...
                  <TextInput
                    ...
                    autoCapitalize="none"
                    keyboardType='numeric'
                    onChangeText={NumberOfTime => {
                      if (Number(NumberOfTime) === 1) {
                        this.setState({ showReminder2: false, showReminder3: false })
                      }
                      if (Number(NumberOfTime) === 2) {
                        this.setState({ showReminder2: true, showReminder3: false })
                      }
                      else {
                        this.setState({ showReminder2: true, showReminder3: true })
                      }
                      this.setState({ NumberOfTime })
                    }}
                    value={NumberOfTime}
                  />
                  ...
                  <DropDownPicker
                    ...
                    value={Meal}
                    items={[{ label: translate('BEFORE MEAL'), value: false }, { label: translate('AFTER MEAL'), value: true }]}
                ...
                <View style={{ flexDirection: 'row' }}>
                  ...
                  <DatePicker
                    ...
                    locale="en_GB"
                    onConfirm={(date) => {
                      this.setState({ Reminder1: date, isFirstDatePickerVisible: false })
                    }}
                    onCancel={() => {
                      this.setState({
                        isFirstDatePickerVisible: false,
                  ...
                  ? (<View style={{ flexDirection: 'row', marginTop: 30 }}>
                    ...
                    <View
                      ...
                      <FontAwesome5
                        ...
                        color="#2196f3"
                        onPress={() => {
                          console.log('showSecondReminder')
                          this.setState({ showSecondReminder: false })
                        }}
                      />
                    </View>
                    ...
                    <DatePicker
                      ...
                      locale="en_GB"
                      onConfirm={(date) => {
                        this.setState({ Reminder2: date, isSecondDatePickerVisible: false })
                      }}
                      onCancel={() => {
                        this.setState({
                          isSecondDatePickerVisible: false,
                ...
                {showReminder3
                  ? (<View style={{ flexDirection: 'row', marginTop: 30 }}>
                    <View
                      ...
                      <Text style={styles.inputTitle}>
                        {translate('THIRD REMINDER AT')} {this.convertDateObjectToTimeString(Reminder3)}
                      </Text>
                    </View>
                    <View
                      style={{
                      ...
                      <FontAwesome5
                        ...
                        size={26}
                        color="#2196f3"
                        onPress={() => this.setState({ showReminder3: false })}
                      />
                    </View>

                    <DatePicker
                      modal
                      open={this.state.isThirdDatePickerVisible}
                      mode='time'
                      date={Reminder3}
                      minuteInterval={30}
                      locale="en_GB"
                      onConfirm={(date) => {
                        this.setState({ Reminder3: date, isThirdDatePickerVisible: false })
                      }}
                      onCancel={() => {
                        console.log('hi')
                        this.setState({
...
export default connect(mapStateToProps, { setLoading })(cgAddMedication);