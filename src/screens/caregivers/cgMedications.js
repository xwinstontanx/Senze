...
class cgMedications extends Component {
  _isMounted = false;
  state = {
    medicationData: [],
    isFetching: false,
  ...
  getData = () => {
    ...
    firestore()
      ...
      .where('CreatedBy', '==', this.state.senior.data.Uid)
      .get()
      .then(snapshot => {
        if (!snapshot.empty) {
          let medications = snapshot.docs;
  ...
  edit(item) {
    this.props.navigation.navigate('cgaddmedication', { refreshList: this.getData, id: item.id, senior: this.state.senior })
  }
    ...
    return (
      <ImageBackground
        ...
        <FloatingAction
          ...
          onPressItem={() => {
            this.props.navigation.navigate('cgaddmedication', { refreshList: this.getData, senior: this.state.senior })
          }}
        />
      </ImageBackground>
    );
...
export default connect(mapStateToProps)(cgMedications);
